"use client"

import {
    Sidebar,
    SidebarContent,
    SidebarFooter,
    SidebarGroup,
    SidebarGroupContent,
    SidebarGroupLabel,
    SidebarMenu,
    SidebarMenuButton,
    SidebarMenuItem,
    SidebarHeader,
    SidebarTrigger,
    useSidebar,
} from "@/components/ui/sidebar"
import { Switch } from "./ui/switch"
import { Label } from "./ui/label"
import {
    Home,
    Search,
    Settings,
    Users,
    FileText,
    BarChart,
    User,
} from "lucide-react"
import Moon from "@/public/icons/moon.svg";
import Image from "next/image";
import Sun from "@/public/icons/sun.svg";
import { useTheme } from "@/contexts/ThemeContext"


export default function AppSidebar() {

    const { state, toggleSidebar } = useSidebar();
    const { toggleTheme } = useTheme();


    return (
        <>
            <Sidebar
                className={`bg-sidebar static h-[calc(100vh-4rem)] duration-300 ease-in-out transition-all overflow-hidden ${state === "expanded" ? "w-50 p-2 border-r border-sidebar-accent" : "w-0 p-0 border-r-0 "
                    }`}
            >

                <SidebarContent>
                    {/* Main Navigation */}
                    <SidebarGroup>
                        <SidebarGroupLabel>ALL BOARDS</SidebarGroupLabel>

                        <SidebarGroupContent>
                            <SidebarMenu>
                                <SidebarMenuItem>
                                    <SidebarMenuButton asChild>
                                        <a href="/">
                                            <Home className="h-4 w-4" />
                                            <span>Home</span>
                                        </a>
                                    </SidebarMenuButton>
                                </SidebarMenuItem>
                                <SidebarMenuItem>
                                    <SidebarMenuButton asChild>
                                        <a href="/search">
                                            <Search className="h-4 w-4" />
                                            <span>Search</span>
                                        </a>
                                    </SidebarMenuButton>
                                </SidebarMenuItem>
                                <SidebarMenuItem>
                                    <SidebarMenuButton asChild>
                                        <a href="/users">
                                            <Users className="h-4 w-4" />
                                            <span>Users</span>
                                        </a>
                                    </SidebarMenuButton>
                                </SidebarMenuItem>
                            </SidebarMenu>
                        </SidebarGroupContent>
                    </SidebarGroup>

                </SidebarContent>


                <SidebarFooter>
                    <SidebarMenu>

                        <SidebarMenuItem className="w-full flex gap-4 justify-center bg-sidebar-accent p-3 rounded-md">
                            <Label htmlFor="airplane-mode"> <Image src={Sun} alt="moon" width={20} height={20} className="cursor-pointer w-4 h-4" /> </Label>

                            <Switch className="data-[state=unchecked]:bg-primary" onClick={() => toggleTheme()} id="airplane-mode" />
                            <Label htmlFor="airplane-mode"> <Image src={Moon} alt="moon" width={20} height={20} className="cursor-pointer w-4 h-4" /> </Label>
                        </SidebarMenuItem>


                        <SidebarTrigger className="flex w-full justify-start mt-2 p-4" />
                    </SidebarMenu>
                </SidebarFooter>
            </Sidebar>


            {/* Sidebar Trigger Button */}
            <div className={`fixed left-0 bottom-4 duration-300 bg-primary/80 hover:bg-primary/50 cursor-pointer rounded-r-2xl p-3  ease-in-out transition-all ${state === "expanded" ? "-translate-x-full" : "translate-x-0"}`} onClick={() => {

                toggleSidebar()
            }}>

                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="11" viewBox="0 0 16 11" fill="none">
                    <path fillRule="evenodd" clipRule="evenodd" d="M15.8154 4.43419C14.2491 1.77636 11.328 0 8 0C4.67056 0 1.75012 1.77761 0.184624 4.43419C-0.0615413 4.8519 -0.0615413 5.37033 0.184624 5.78805C1.75087 8.44585 4.67195 10.2222 8 10.2222C11.3294 10.2222 14.2499 8.4446 15.8154 5.78802C16.0615 5.37031 16.0615 4.85189 15.8154 4.43419ZM8 8.88887C5.91217 8.88887 4.22223 7.19924 4.22223 5.1111C4.22223 3.02327 5.91184 1.33333 8 1.33333C10.0878 1.33333 11.7778 3.02294 11.7778 5.1111C11.7778 7.19893 10.0882 8.88887 8 8.88887ZM8 7.99999C9.5955 7.99999 10.8889 6.7066 10.8889 5.1111C10.8889 3.51561 9.5955 2.22222 8 2.22222C7.50811 2.22222 7.04503 2.3453 6.63964 2.56211L6.64053 2.56208C7.2975 2.56208 7.83008 3.09466 7.83008 3.75163C7.83008 4.40858 7.2975 4.94116 6.64053 4.94116C5.98356 4.94116 5.45098 4.4086 5.45098 3.75163L5.451 3.75074C5.2342 4.15613 5.11112 4.61921 5.11112 5.1111C5.11112 6.7066 6.4045 7.99999 8 7.99999Z" fill="white" />
                </svg>

            </div >
        </>
    )
}
